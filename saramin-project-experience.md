...
- 정량 지표가 확인된 RF-DETR, VAE, FullCount는 기존 포트폴리오 자료 기준임을 전제로 사용할 수 있습니다.
...
- GraphRAG와 re-ranking은 실제 구현 완료가 아니라 고도화 방향으로 분리했습니다.
...
- Structural Pruning을 적용해 제한된 환경에서도 실시간 추론에 가까운 속도를 확보하는 방향으로 개선했습니다.
- 기존 포트폴리오 자료 기준 mAP 약 7%p 개선과 22 FPS 이상 추론 성능을 기록했습니다.

### 보수적 표현

- 의료 영상 특성에 맞춘 데이터 증강과 모델 경량화 관점으로 대장 용종 검출 프로젝트를 수행했습니다.
...
- Structural Pruning 기반 추론 속도 개선
- mAP 약 7%p 개선, 22 FPS 이상 추론 성능 기록
...
- KLD와 MSE를 조합한 커스텀 손실 함수를 설계해 정상 패턴 복원과 이상 탐지 민감도 사이의 균형을 조정했습니다.
...
- API Polling 방식의 상태 확인을 이벤트 기반 구조로 전환해 기존 포트폴리오 자료 기준 DB I/O를 약 80% 줄였습니다.
