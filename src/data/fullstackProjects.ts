    ...
    details: [
      ...
      },
      {
        ...
        problemSolving: [
          {
            label: "측정 현상",
            text: "기존 HTTP 동기 요청 구조에서는 위험 발생 후 관제 화면 수신까지 최대 수 초의 지연이 발생하였으며, 네트워크 지연 시 경보 누락 현상이 관찰되었습니다.",
          },
          {
            label: "원인 분석",
            text: "서버 요청 로그 분석 결과, 동기식 HTTP 요청 처리 비용과 브라우저 폴링(Polling) 간격으로 인해 알림 전달 지연이 발생하고 있었습니다.",
          },
          {
            label: "의사결정",
            text: "관제 서비스의 핵심인 실시간성과 유실 없는 데이터 저장을 보장하기 위해, MQTT QoS 1 및 WebSocket·STOMP 기반의 비동기 알림 파이프라인을 구축하기로 결정했습니다.",
          },
          {
            label: "구현",
            text: "Spring Boot Subscriber에서 이벤트를 비동기로 수신하여 DB에 즉시 저장하고, 시설별 STOMP 토픽을 통해 React 관제 화면으로 실시간 알림을 브로드캐스트했습니다.",
          },
          {
            label: "결과",
            text: "2카메라 TensorRT 환경 테스트 기준, End-to-End 알림 지연 시간을 평균 20.9ms(p95 26ms)로 단축하였으며 29건 모두 1초 이내 전송(전달 성공률 100%)을 달성했습니다.",
          },
          {
            label: "배운 점",
            text: "네트워크 브로드캐스트 성능뿐만 아니라, MQTT QoS 1의 재전송 특성에 대비한 멱등성(Idempotency) 설계가 실시간 시스템의 안정성을 결정한다는 점을 배웠습니다.",
          },
        ],
        note:
      ...
      },
      {
        ...
        problemSolving: [
          {
            label: "측정 현상",
            text: "단일 위험 상황 발생 시, 백엔드 DB 및 프론트엔드 관제 화면에 동일한 사고 카드가 3~4건 중복 생성되어 관제 효율을 저하시켰습니다.",
          },
          {
            label: "원인 분석",
            text: "DB 저장 로그 및 WebSocket 패킷 추적 결과, 최초 경보 이벤트와 이후 비동기로 도착한 미디어(스냅샷·클립) 이벤트의 식별자가 달라 별개의 사고 건으로 처리되고 있었습니다.",
          },
          {
            label: "의사결정",
            text: "프론트엔드 단의 단순 필터링 방식 대신, DB 데이터 정합성과 화면 상태를 근본적으로 일치시키기 위해 단일 식별자(originalEventId) 기반의 통합 병합 구조를 설계했습니다.",
          },
          {
            label: "구현",
            text: "originalEventId를 유일한 사고 키로 지정하여, 백엔드에서는 기존 사고 엔티티에 미디어를 연결(Upsert)하고 프론트엔드에서는 동기/비동기 응답을 동일 사고 카드로 자동 병합하도록 구현했습니다.",
          },
          {
            label: "결과",
            text: "비동기 데이터의 도착 순서가 불일치하는 상황에서도 사고 중복 생성률 0%를 달성하였으며, 모든 증거 미디어가 단일 사고 건으로 정확히 통합 관리되도록 개선했습니다.",
          },
          {
            label: "배운 점",
            text: "비동기 분산 파이프라인에서는 데이터가 순서대로 도착하지 않는다는 전제하에, 백엔드와 프론트엔드가 공유하는 명확한 식별자와 병합 규칙을 수립해야 함을 배웠습니다.",
          },
        ],
        images: [
          {
      ...
      },
      {
        ...
        problemSolving: [
          {
            label: "측정 현상",
            text: "AI 설명 생성(VLM) 및 768차원 임베딩 추출 작업을 동기로 처리할 때, 외부 API 응답 지연(수 초~수십 초)으로 인해 긴급 경보 알림 전송이 차단되는 문제가 발생했습니다.",
          },
          {
            label: "원인 분석",
            text: "시스템 로그 분석 결과, 이미지 키프레임 추출, 얼굴 비식별화, Gemini VLM 호출 등 비중 있는 연산들이 실시간 경보 발송의 핵심 경로(Critical Path)를 블로킹하고 있었습니다.",
          },
          {
            label: "의사결정",
            text: "즉각적인 반응성이 요구되는 실시간 경보 경로와, 사고 후 분석을 위한 VLM 문맥 처리 파이프라인을 완전 비동기 백그라운드 작업 큐로 분리하기로 결정했습니다.",
          },
          {
            label: "구현",
            text: "사고 저장 및 STOMP 알림 전송을 1차로 즉시 완료한 후, 백그라운드 Worker에서 키프레임 추출·비식별화·Gemini VLM 분석·pgvector 임베딩 저장을 작업 상태 관리 및 재시도(지수 백오프) 로직과 함께 구현했습니다.",
          },
          {
            label: "결과",
            text: "실시간 경보 지연 시간을 20ms 대 수준으로 유지하는 동시에, 비동기 파이프라인을 통해 자연어 기반의 사고 문맥 검색(RAG) 기능을 안정적으로 제공했습니다.",
          },
          {
            label: "배운 점",
            text: "서비스의 핵심 SLA(서비스 수준 합약)를 준수하기 위해서는 부하가 큰 후처리 작업을 메인 흐름에서 비동기로 분리(Decoupling)하고 예외 재시도 구조를 갖춰야 함을 배웠습니다.",
          },
        ],
        images: [
          {
