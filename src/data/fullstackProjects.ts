  ...
  },
  {
    ...
    details: [
      ...
      },
      {
        title: "판단과 배운 점",
        items: [
          "**실시간 알림과 VLM 분석을 분리**: VLM 설명과 임베딩 생성을 기다린 뒤 알림을 보내면 긴급 경보가 늦어질 수 있었습니다. 위험 이벤트는 MQTT로 먼저 전달하고 VLM 분석은 사고 저장 이후 비동기로 처리했습니다. 이를 통해 모든 기능을 하나의 경로에 넣기보다 즉시 필요한 기능과 이후 보강할 기능을 분리해야 한다는 점을 배웠습니다.",
          "**화면의 중복이 아닌 데이터 흐름 전체를 수정**: 실시간 알림과 나중에 도착한 영상 이벤트가 서로 다른 ID를 사용해 하나의 사고가 여러 건으로 표시됐습니다. 프론트에서 중복을 숨기는 데 그치지 않고 `originalEventId`를 기준으로 백엔드 저장과 프론트 상태 병합을 함께 수정했습니다. 이를 통해 비동기 시스템은 데이터가 순서대로 도착하지 않는다는 전제로 식별자와 병합 규칙을 설계해야 한다는 점을 배웠습니다.",
        ],
      },
      {
