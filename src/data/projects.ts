...
export const projects: readonly Project[] = [
  {
    ...
    details: [
      ...
      },
      {
        title: "판단과 배운 점",
        items: [
          "**현재 상황을 우선하는 처리 정책**: 모든 프레임을 보존하자 과거 영상이 누적되어 위험 알림이 늦어졌습니다. 안전 관제의 목적은 영상을 빠짐없이 처리하는 것이 아니라 현재 위험을 빠르게 발견하는 것이라고 판단해 오래된 프레임을 폐기했습니다. 이 경험을 통해 데이터 손실 여부보다 서비스 목적에 맞는 처리 기준을 선택하는 것이 중요하다는 점을 배웠습니다.",
          "**개별 모델보다 전체 파이프라인 기준으로 선택**: YOLO Pose의 FPS만으로 모델을 선택하면 최종 LSTM 행동 분류 성능을 설명하기 어려웠습니다. 실제 위험을 덜 놓치는 것이 중요하다고 판단해 관절 시퀀스를 LSTM에 입력한 뒤 Faint Recall과 F1-score까지 비교했습니다. 이를 통해 모델 선택은 개별 벤치마크가 아니라 전체 서비스 결과를 기준으로 해야 한다는 점을 배웠습니다.",
        ],
      },
      {
  ...
  },
  {
    ...
    GUI --> Result["Video/Webcam Detection\\nmAP@50 86.2%"]`,
      ...
      },
      {
        title: "판단과 배운 점",
        items: [
          "**모델 교체보다 데이터 품질을 먼저 개선**: 내시경 영상의 조명과 병변 형태 차이로 일반화가 어려웠지만, 곧바로 더 큰 모델을 사용하는 대신 데이터 다양성을 먼저 확보해야 한다고 판단했습니다. Elastic Deformation과 Grid Distortion을 적용해 실제 조직의 형태 변화를 반영했습니다. 이 과정에서 성능 문제의 원인이 항상 모델 구조에 있는 것은 아니며 데이터 설계가 먼저 검토돼야 한다는 점을 배웠습니다.",
          "**증강 효과보다 Bounding Box 정합성을 우선**: 영상만 변형하고 라벨이 정확히 따라가지 않으면 학습 데이터 자체가 잘못될 수 있었습니다. 증강 결과가 화려하게 보이는 것보다 변형된 병변과 Bounding Box가 일치하는지 직접 검증하는 것을 우선했습니다. 이를 통해 데이터 증강은 이미지 개수를 늘리는 작업이 아니라 입력과 정답의 의미를 함께 보존하는 과정이라는 점을 배웠습니다.",
        ],
      },
      {
  ...
  },
  {
    ...
    Mask --> BBox["🎯 Anomaly Detection\\nCandidate Region"]`,
      ...
      },
      {
        title: "판단과 배운 점",
        items: [
          "**라벨 부족을 모델 문제가 아닌 문제 정의로 접근**: 병변 위치 라벨이 부족한 상태에서 지도학습 모델을 그대로 적용하기는 어려웠습니다. 정상 조직의 분포를 먼저 학습하고 정상에서 벗어난 영역을 찾는 비지도 이상 탐지로 문제를 다시 정의했습니다. 이를 통해 데이터 제약을 무시하고 모델을 선택하기보다 확보 가능한 데이터에 맞춰 문제 자체를 재구성해야 한다는 점을 배웠습니다.",
          "**고정 임계값보다 영상별 분포를 반영**: 초음파 영상마다 밝기와 노이즈가 달라 같은 임계값에서도 오탐 수준이 크게 달라졌습니다. 모든 영상에 동일한 기준을 강제하기보다 이미지별 Reconstruction Error 분포를 반영하는 Dynamic Threshold를 적용했습니다. 이 경험을 통해 후처리 역시 모델 성능의 일부이며 입력 환경의 변화를 고려해야 한다는 점을 배웠습니다.",
        ],
      },
      {
        title: "이 경험으로 수행할 수 있게 된 기술 업무",
  ...
  },
  {
    ...
    Context --> Answer["✅ Answer + Source Evidence"]`,
      ...
      },
      {
        title: "판단과 배운 점",
        items: [
          "**벡터 검색 하나로 모든 질문을 해결하지 않았습니다**: 의미가 비슷한 문서를 찾는 데는 Vector Search가 유리하지만 cameraLoginId, frameId와 같은 정확한 식별자는 누락될 수 있었습니다. 키워드에 강한 BM25와 문맥에 강한 Vector Search를 결합하고 RRF로 순위를 통합했습니다. 이를 통해 검색 기술은 하나를 선택하는 문제가 아니라 질문 유형별 강점을 조합하는 문제라는 점을 배웠습니다.",
          "**구현 완료와 검색 품질 검증을 구분했습니다**: 하이브리드 검색 구조를 구현했더라도 실제 검색 품질이 향상됐다고 단정하려면 Recall@k와 MRR 같은 평가가 필요했습니다. 아키텍처 구현 결과와 정량 검증 결과를 분리해 설명하고, 부족한 평가는 후속 과제로 명시했습니다. 이를 통해 구현 사실과 검증된 성과를 구분하는 태도가 기술 신뢰도를 높인다는 점을 배웠습니다.",
        ],
      },
      {
        title: "이 경험으로 수행할 수 있게 된 기술 업무",
