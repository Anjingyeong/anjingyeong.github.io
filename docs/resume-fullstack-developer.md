...
사용자의 한 번의 입력이 화면, API, 데이터와 결과까지 막힘없이 이어지도록 만드는 개발자입니다. 스마트 안전 관제에서 AI 이벤트가 MQTT·백엔드·WebSocket을 거쳐 관제 화면에 도달하는 실시간 흐름을 연결했고, 마음이음 웹서비스는 기획부터 배포까지 1인 개발했습니다. 화면과 서버를 따로 구현하기보다, 데이터가 이동하는 과정과 예외 상황을 함께 살피며 서비스를 완성합니다. 특정 기술에 갇히기보다 요구사항에 필요한 구조를 빠르게 파악하고, 아이디어를 실제로 작동하는 제품으로 바꿀 수 있는 사람입니다.
...
### 마음이음 | 개인정보 최소 수집형 자가체크 및 결과 리포트 웹서비스 ([서비스 링크](https://maumium.pages.dev/))
...
- 스마트 안전관제 개발 기록을 50개 문서와 737개 Chunk의 검색 가능한 지식으로 구조화했습니다.
- 키워드 검색에는 BM25, 문맥 검색에는 Vector Search를 적용하고 RRF로 결과 순위를 결합했습니다.
- 61개 Golden Query로 검색 품질을 측정해 Legacy Hybrid Hit@5 82.14%, Recall@5 61.01%, MRR 0.6875를 기준선으로 확보했습니다.
- Elasticsearch BM25·dense_vector kNN·HNSW·Bulk Indexing·RRF Provider를 기존 Legacy 경로와 분리해 구현했습니다.
- 신규 테스트 3개와 전체 회귀 테스트 92개를 통과했으며, 실제 Elasticsearch 전체 질의 실측은 후속 검증으로 구분했습니다.
