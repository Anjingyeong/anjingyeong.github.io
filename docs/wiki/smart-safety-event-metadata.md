...
The intended retrieval path is metadata filtering first, then BM25 keyword search and vector search, combined with RRF ranking. Optional re-ranking can be added after the corpus grows. GraphRAG remains a later candidate only after document relationships and scale justify it.
