...
- **기간:** 2025.03 - 2025.11
- **역할:** 모델 실험, 의료 영상 데이터 증강, 모델 경량화 및 추론 검증
...
- Structural Pruning을 적용해 제한된 환경에서도 추론 속도를 확보하는 방향으로 개선했습니다.
- 기존 포트폴리오 자료 기준 mAP 약 7%p 개선, 22 FPS 이상 추론 성능을 기록했습니다.
- 관련 프로젝트로 건양대학교 캡스톤디자인 경진대회와 전국 공학교육혁신 컨소시엄 경진대회 수상 경험이 있습니다.

### VAE 기반 유방 초음파 이상 탐지

- **기간:** 2024.03 - 2024.10
- **역할:** 이상 탐지 흐름 설계, 커스텀 손실 함수 구현, 재구성 오차 기반 후처리
...
- KLD와 MSE를 조합한 손실 함수를 사용해 정상 패턴 복원과 이상 영역 탐지 사이의 균형을 조정했습니다.
...
- 기존 포트폴리오 자료 기준 DB I/O를 약 80% 줄였고, 거래 상태 변경이 프론트엔드에 실시간 반영되는 흐름을 구현했습니다.
...
- RF-DETR 프로젝트에서는 의료 영상의 왜곡과 조명 변화를 데이터 증강으로 보완하고, 제한된 환경에서도 추론 속도를 확보하는 방향으로 개선했습니다.
